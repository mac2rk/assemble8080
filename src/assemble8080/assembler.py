...
def substitute(text, mapping, short=False):
    ...
    # PRINTX uses its own delimiter: semicolons in its payload are literal.
    # Apply this at every expansion level, including REPT/IRP/IRPC bodies.
    if not re.match(r'^\s*\.PRINTX(?:\s|$)', text, re.I):
